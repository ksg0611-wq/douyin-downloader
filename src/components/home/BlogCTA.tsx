...
import { Download, Sparkles, TrendingUp, ShieldAlert, Languages, Mail } from "lucide-react";
...
export default function BlogCTA({ category = "" }: BlogCTAProps) {
  ...
  let link = "/downloader?scroll=downloader";
  let icon = <Download className="w-5 h-5 text-white animate-bounce" />;
  let gradient = "from-[#00f2fe] via-[#5c64ff] to-[#fe0979]";

  if (category.includes("기획") || category.includes("분석") || category.includes("역설계")) {
    title = "AI 바이럴 영상 역설계 분석기";
    desc = "잘 터지는 경쟁사 숏폼 영상의 비밀을 파헤칩니다. 대본의 후킹 포인트와 이탈 방지 기법을 AI로 10초 만에 완벽히 분석하세요.";
    btnText = "🔍 AI 바이럴 역설계 분석하기";
    link = "/viral-analyzer";
    icon = <Sparkles className="w-5 h-5 text-white" />;
    gradient = "from-fuchsia-600 via-pink-600 to-rose-500";
  } else if (category.includes("마케팅") || category.includes("해시태그") || category.includes("트렌드")) {
    title = "실시간 해시태그 트렌드 분석기";
    desc = "중국 및 글로벌 숏폼 해시태그의 유입량, 경쟁도, 연관 바이럴 태그를 AI로 정밀 분석하여 노출 확률을 높이세요.";
    btnText = "🔥 해시태그 트렌드 분석기 써보기";
    link = "/downloader?tool=hashtag";
    ...
    link = "/downloader?tool=hook-generator";
    icon = <Sparkles className="w-5 h-5 text-white" />;
    ...
    link = "/downloader?tool=translator";
    ...
    link = "/downloader?tool=shadowban-scanner";
    icon = <ShieldAlert className="w-5 h-5 text-white" />;
    gradient = "from-rose-500 to-red-600";
  } else if (category.includes("협찬") || category.includes("제안서") || category.includes("수익화")) {
    title = "AI 브랜드 협찬 제안서 자동 생성기";
    desc = "내 채널의 주요 시청자층 정보와 강점을 입력하여 브랜드 마케터의 마음을 움직이는 비즈니스 메일 제안서를 10초 만에 작성해 보세요.";
    btnText = "💌 AI 협찬 제안서 생성기 실행하기";
    link = "/downloader?tool=sponsor-pitch-generator";
    icon = <Mail className="w-5 h-5 text-white" />;
    gradient = "from-emerald-500 via-teal-650 to-indigo-600";
  } else if (category.includes("알고리즘") || category.includes("아웃트로") || category.includes("CTA")) {
    title = "AI 알고리즘 폭발 CTA & 댓글 유도 멘트 생성기";
    desc = "시청자의 행동(저장, 공유, 좋아요, 댓글)을 폭발적으로 유도하는 아웃트로 CTA 멘트와 고정 댓글용 질문 10종을 AI로 10초 만에 제작해 보세요.";
    btnText = "📢 AI CTA 생성기 실행하기";
    link = "/downloader?tool=algo-hook-generator";
    icon = <Sparkles className="w-5 h-5 text-white" />;
