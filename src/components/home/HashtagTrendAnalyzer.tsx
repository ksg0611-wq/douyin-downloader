...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-zinc-900/95 border border-zinc-700 rounded-xl px-3 py-2.5 text-xs shadow-2xl backdrop-blur-xl">
...
function getCompetitionStyle(competition: string) {
  if (competition === "상") return "bg-rose-500/20 text-rose-300 border-rose-500/30";
  if (competition === "중") return "bg-amber-500/20 text-amber-300 border-amber-500/30";
      ...
      <div className="bg-zinc-950/60 border border-zinc-800 rounded-2xl p-5 sm:p-6 backdrop-blur-xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative flex flex-col sm:flex-row gap-2.5 mb-4">
          <div className="relative flex-grow group">
            ...
            <div className="relative flex items-center bg-zinc-900 border border-zinc-800 rounded-xl">
              <Hash className="w-4 h-4 text-zinc-400 ml-3.5 shrink-0" />
              <input
                ...
                className="w-full bg-transparent text-zinc-100 placeholder-zinc-500 text-sm px-3 py-3.5 focus:outline-none font-sans"
              />
            </div>
          </div>
          <button
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          {QUICK_KEYWORDS.map((kw) => (
            <button
              ...
              className="px-2.5 py-1 rounded-full bg-zinc-800/80 hover:bg-zinc-700 border border-zinc-700/60 text-zinc-400 hover:text-zinc-100 text-[11px] font-bold transition-all cursor-pointer"
            >
              #{kw}
            </button>
          ))}
        ...
        <AnimatePresence>
          {error && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className="flex items-center gap-2 text-rose-400 text-xs font-medium mb-4 px-1"
            >
        ...
        <AnimatePresence>
          {isLoading && (
            <motion.div
              ...
              className="space-y-3"
            >
              <div className="h-5 bg-zinc-800/60 rounded-lg animate-pulse w-1/3" />
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800/60 pb-4">
                <div className="flex items-center gap-2">
                  <span className="text-lg font-black text-white">#{result.keyword}</span>
                  <span className="text-xs text-zinc-500">7일 트렌드 분석 결과</span>
                </div>
                <div className="flex items-center gap-2">
                  <Zap className="w-4 h-4 text-yellow-400" />
                  <span className="text-xs text-zinc-400 font-medium">트렌드 지수</span>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={result.chartData} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                      ...
                      <Area
                        ...
                        dot={false}
                        activeDot={{ r: 5, fill: "#00f2fe", stroke: "#0e0e12", strokeWidth: 2 }}
              ...
              <div className="grid grid-cols-3 gap-2.5">
                ...
                <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-3 text-center">
                  ...
                  <span className="text-[10px] text-zinc-600 block">누적</span>
                </div>
                {/* 경쟁 강도 */}
                <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-3 text-center">
                  ...
                  <span className={`text-base sm:text-lg font-black font-mono px-2 py-0.5 rounded-lg border ${getCompetitionStyle(result.competition)}`}>
                    {result.competition} ({result.competitionEn})
                  </span>
                </div>
                {/* 트렌드 방향 */}
                <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-3 text-center">
              ...
              <div className="space-y-2">
                {result.insights.map((insight, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-2.5 bg-violet-950/20 border border-violet-500/15 rounded-xl px-3.5 py-2.5 text-xs text-violet-200 leading-relaxed"
                  >
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-zinc-400 hover:text-white transition-all cursor-pointer"
                  >
                    {copiedAll ? (
                      <>
                        <CheckCircle2 className="w-3 h-3 text-emerald-400" />
                        <span className="text-emerald-400">전체 복사됨</span>
                      </>
                    ) : (
                      <>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-bold transition-all cursor-pointer ${
                        copiedTag === tag
                          ...
                          : "bg-zinc-800/80 border-zinc-700/60 text-zinc-300 hover:bg-violet-600/20 hover:border-violet-500/40 hover:text-violet-200"
                      }`}
                    >
                      {copiedTag === tag ? (
              ...
              <button
                onClick={() => handleAnalyze()}
                className="w-full mt-1 py-2.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 text-zinc-400 hover:text-zinc-200 font-bold text-xs rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all"
              >
