...
export default function UploadTimeCalculator({ lang = "ko" }: UploadTimeCalculatorProps) {
  ...
  const handleSaveToToolbox = async () => {
    ...
    try {
      ...
      console.error("[UploadTimeCalculator] Firestore 저장 실패:", err);
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-lg font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-2">
            🌍 숏폼 업로드 타임 계산기 (Global Upload Time Calculator)
          ...
          <p className="text-xs text-zinc-650 dark:text-zinc-400 mt-1">
            본 도구는 글로벌 타겟 국가의 활성 유저 접속 피크 시간을 연계하여 최적의 업로드 예약 타임을 한국 표준시(KST)로 변환 산출합니다. 목표 국가와 주요 플랫폼을 정하면 국가별 시차와 요일별 활성 트래픽 데이터를 대조 분석하여 가장 반응이 좋은 최적의 예약 업로드 시각을 제공합니다.
          </p>
        </div>
      </div>
