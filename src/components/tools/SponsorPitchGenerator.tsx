...
export default function SponsorPitchGenerator({ lang = "ko" }: SponsorPitchGeneratorProps) {
  ...
  const handleSaveToToolbox = async () => {
    ...
    try {
      ...
      console.error("[SponsorPitchGenerator] Firestore 저장 실패:", err);
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-lg font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-2">
            💰 브랜드 협찬(광고) 제안서 자동 생성기 (AI Sponsor Pitch Generator)
          ...
          <p className="text-xs text-zinc-650 dark:text-zinc-400 mt-1">
            본 도구는 내 채널의 상세 스펙과 타겟 브랜드 특성을 종합해 비즈니스 협찬을 이끌어낼 수 있는 정밀한 콜드메일 제안서를 완성합니다. 브랜드 정보와 타겟 독자층을 기입하면, AI가 마케팅 협업 의사를 타진하기에 최적화된 격식 있고 설득력 높은 커머셜 제안서를 생성합니다.
          </p>
        </div>
      </div>
