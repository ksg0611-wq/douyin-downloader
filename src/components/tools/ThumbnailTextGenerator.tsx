  ...
  const handleSaveToToolbox = async () => {
    ...
    try {
      await addDoc(collection(db, "users", user.uid, "history"), {
        toolId: "thumbnail-text-generator",
        toolName: "0.1초 시선 강탈 썸네일 텍스트 생성기",
        inputData: {
          topic: topic.trim()
        },
        resultData: result,
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-lg font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-2">
            🧲 0.1초 시선 강탈 썸네일 텍스트 생성기 (Thumbnail Copy Generator)
          ...
          <p className="text-xs text-zinc-650 dark:text-zinc-400 mt-1">
            본 도구는 무한 스크롤되는 피드 노출 시 즉각적인 사용자의 클릭 반응을 유도할 수 있는 매운맛/순한맛 썸네일 전용 텍스트 카피를 생성합니다. 영상 주제를 입력하면, 0.1초 안에 각인되어 썸네일 클릭률(CTR)을 폭발시키는 10종의 매력적인 숏텍스트 후보군을 도출합니다.
          </p>
        </div>
      </div>
